...
impl Builder {
    ...
    fn _staging_cert_store(&self) -> Result<rustls::RootCertStore, Error> {
        let mut roots = rustls::RootCertStore::empty();
        if self.pem.is_some() {
            roots.add_server_trust_anchors(
                self.trust_anchors()?.into_iter().map(Into::into)
...
-----END CERTIFICATE-----"
                ).map_err(ClientError::with_source_ref)?.into_iter().map(Into::into)
            );
        }
        Ok(roots)
    }
    ...
    pub fn build(&self) -> Result<Config, Error> {
        ...
        let verifier = match tls_security {
            ...
            Staging => Arc::new(rustls::client::WebPkiVerifier::new(
                self._staging_cert_store()?,
                None,
            )) as Verifier,
