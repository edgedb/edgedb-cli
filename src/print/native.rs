...
impl FormatExt for Value {
    fn format<F: Formatter>(&self, prn: &mut F) -> Result<F::Error> {
        ...
        match self {
            ...
            V::Str(s) => prn.const_scalar(format_string(s, prn.expand_strings())),
                    ...
                    for (fld, value) in shape.elements.iter().zip(fields) {
                        if !fld.flag_implicit || prn.implicit_properties() {
                            if fld.flag_link_property {
                                prn.object_field(
                                    ("@".to_owned() + &fld.name).rgb(0, 0xa5, 0xcb).bold(),
                                )?;
                            } else {
                                prn.object_field(fld.name.clone().light_blue().bold())?;
                            };
                            value.format(prn)?;
                            prn.comma()?;
                            n += 1;
