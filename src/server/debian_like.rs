...
use crate::server::package::{Package, PackageCandidate, RepositoryInfo};
...
fn sources_list(codename: &str, nightly: bool) -> String {
    format!(
        "deb https://packages.edgedb.com/apt {}{} main\n",
        codename,
...
impl Debian {
    ...
    pub fn get_repo(&self, nightly: bool) -> anyhow::Result<Option<&RepositoryInfo>> {
        if nightly {
            self.nightly_repo
                .get_or_try_init(|| {
                    task::block_on(remote::get_json_opt(
                        &format!(
                            "https://packages.edgedb.com/apt/.jsonindexes/\
                            {}.nightly.json",
                            self.codename
                        ),
    ...
    pub fn all_versions(&self, nightly: bool) -> anyhow::Result<Vec<DistributionRef>> {
        Ok(self
            .get_repo(nightly)?
            .map(|x| {
                x.packages
    ...
    pub fn get_available_methods(&self) -> Result<InstallationMethods, anyhow::Error> {
    ...
    pub fn install_operations(
        ...
        operations.push(Operation::PrivilegedCmd(
            Command::new("apt-get").arg("update"), // TODO(tailhook) uncomment them we fix
...
pub fn get_installed() -> anyhow::Result<Vec<DistributionRef>> {
    ...
    for line in out.stdout.split(|&b| b == b'\n') {
        let pkg_name = match str::from_utf8(line)
            .ok()
