...
pub(in crate::server) use operation::{Command, Operation};
...
pub fn install(options: &Install) -> Result<(), anyhow::Error> {
    if cfg!(target_os = "linux") {
        ...
        if do_docker_check {
            docker_check()
                .map_err(|e| log::warn!("Failed to check if running within a container: {:#}", e))
                .ok();
        }
    }
    ...
    let effective_method = options.method.clone().unwrap_or(InstallMethod::Package);
    if !options.interactive && !methods.contains_key(&effective_method) {
    ...
    let mut settings_builder = SettingsBuilder::new(&*current_os, options, methods)?;
