...
use crate::server::options::{Destroy, Logs, Restart, Start, StartConf, Stop};
...
use crate::server::{centos, debian, ubuntu};
    ...
    fn upgrade<'x>(&'x self, meta: &Metadata) -> anyhow::Result<InstanceRef<'x>> {
        Ok(LocalInstance {
            ...
            slot: Lazy::eager(
                meta.slot
                    .as_ref()
                    .expect("linux packages always have a slot")
                    .clone(),
            ),
...
impl CurrentOs for Unknown {
    ...
    fn make_method<'x>(
        ...
        use InstallMethod::*;
        match method {
            Package => anyhow::bail!("Package method is unsupported on {}", self.distro_name),
...
pub fn detect_distro() -> Result<Box<dyn CurrentOs>, anyhow::Error> {
    if Path::new("/etc/os-release").exists() {
        ...
        let data =
            fs::read_to_string("/etc/centos-release").context("Reading /etc/centos-release")?;
        if let Some(dpos) = data.find('.') {
            if data.starts_with("CentOS release ") {
                ...
                return Ok(Box::new(centos::Centos::from_release(release)?));
            }
        }
...
pub fn systemd_service_path(name: &str, system: bool) -> anyhow::Result<PathBuf> {
    Ok(unit_dir(system)?.join(&unit_name(name)))
}

pub fn create_systemd_service(name: &str, meta: &Metadata) -> anyhow::Result<()> {
    let unit_dir = unit_dir(false)?;
    fs::create_dir_all(&unit_dir)?;
    let unit_name = unit_name(name);
    let unit_path = unit_dir.join(&unit_name);
    fs::write(&unit_path, systemd_unit(name, meta)?)?;
    process::run(Command::new("systemctl").arg("--user").arg("daemon-reload"))?;
...
pub fn get_instance<'x>(method: &'x dyn Method, name: &str) -> anyhow::Result<InstanceRef<'x>> {
