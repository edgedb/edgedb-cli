...
use crate::server::upgrade::{BackupMeta, UpgradeMeta};
use crate::table;
            ...
            Ok(meta) => {
                ...
                if let Some(port) = self.reserved_port {
                    if meta.port == port {
                        ...
                        println!("  Port: {} (but {} reserved)", meta.port, port);
                    }
                } else {
...
pub fn probe_port(metadata: &anyhow::Result<Metadata>, reserved: &Option<u16>) -> Port {
    use Port::*;
    ...
    let probe = task::block_on(timeout(
...
pub fn print_status_all(extended: bool, debug: bool, json: bool) -> anyhow::Result<()> {
    let os = detect::current_os()?;
    let methods = os.get_available_methods()?.instantiate_all(&*os, true)?;
    let mut statuses = Vec::new();
    for meth in methods.values() {
        statuses.extend(meth.all_instances()?.into_iter().map(|i| i.get_status()));
    }
