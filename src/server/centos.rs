...
use crate::server::options::{Destroy, Upgrade};
use crate::server::os_trait::{CurrentOs, InstanceRef, Method};
use crate::server::package::{self, Package, PackageMethod};
use crate::server::package::{PackageCandidate, RepositoryInfo};
...
impl Centos {
    pub fn new(rel: &os_release::OsRelease) -> anyhow::Result<Centos> {
        let release = rel
            .version_id
            .parse()
            .with_context(|| format!("Error parsing version {:?}", rel.version_id))?;
            ...
            self.nightly_repo
                .get_or_try_init(|| {
                    task::block_on(remote::get_json_opt(
                        &format!(
                            "https://packages.edgedb.com/rpm/.jsonindexes/\
                        el{}.nightly.json",
                            self.release
                        ),
    ...
    fn install_operations(&self, settings: &install::Settings) -> anyhow::Result<Vec<Operation>> {
        let pkg = settings
            .distribution
        ...
        let update_list = match fs::read(&repo_path) {
            ...
            Err(e) => {
                log::warn!("Unable to read {}: {:#}. Will replace...", repo_path, e);
                true
            }
        };
        if update_list {
    ...
    fn uninstall_operations(&self, distr: &DistributionRef) -> anyhow::Result<Vec<Operation>> {
        let pkg = distr
            .downcast_ref::<Package>()
            .context("invalid centos package")?;
        let mut operations = Vec::new();
