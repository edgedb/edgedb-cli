...
use crate::server::detect::{Lazy, VersionQuery, ARCH};
...
use crate::server::install::{self, Command, Operation};
...
use crate::server::package::{Package, PackageMethod};
...
impl Macos {
    fn get_repo(&self, nightly: bool) -> anyhow::Result<Option<&RepositoryInfo>> {
        if nightly {
            self.nightly_repo
                .get_or_try_init(|| {
                    task::block_on(remote::get_json_opt(
                        ...
                        .jsonindexes/macos-{}.nightly.json",
                            ARCH
                        ),
            ...
            self.stable_repo
                .get_or_try_init(|| {
                    Ok(task::block_on(remote::get_json_opt(
                        &format!(
                            "https://packages.edgedb.com/archive/\
                        .jsonindexes/macos-{}.json",
                            ARCH
                        ),
                        "failed to fetch repository index",
                    ))?
                    .map(|mut repo: RepositoryInfo| {
                        repo.packages.retain(|p| {
...
impl<'os> Method for PackageMethod<'os, Macos> {
    ...
    fn install(&self, settings: &install::Settings) -> Result<(), anyhow::Error> {
        ...
        task::block_on(remote::get_file(&pkg_path, &url)).context("failed to download package")?;
        ...
        self.os
            .unix
            .perform(operations, "installation", "edgedb server install")?;
        Ok(())
    }
    fn uninstall(&self, distr: &DistributionRef) -> Result<(), anyhow::Error> {
        let pkg = distr
            .downcast_ref::<Package>()
            .context("invalid macos package")?;
        let entries = get_package_paths(&pkg)?;
        let operations = vec![
            Operation::PrivilegedCmd(Command::new("rm").arg("-rf").args(entries)),
        ...
        self.os
            .unix
            .perform(operations, "uninstallation", "edgedb server uninstall")?;
        Ok(())
    }
    ...
    fn installed_versions(&self) -> anyhow::Result<Vec<DistributionRef>> {
        Ok(self
            ...
            .get_or_try_init(|| {
                ...
                let out = cmd.output().context("cannot get installed packages")?;
                if out.status.code() == Some(1) {
                    return Ok(Vec::new());
                } else if !out.status.success() {
                    anyhow::bail!("cannot get installed packages: {:?} {}", cmd, out.status);
                }
                ...
                for line in lines {
                    ...
                    let out = cmd.output().context("cannot get package version")?;
                    if !out.status.success() {
                        anyhow::bail!("cannot get package version: {:?} {}", cmd, out.status);
                    }
                    let lines = out
                        .stdout
                        .split(|&b| b == b'\n')
    ...
    fn destroy(&self, options: &Destroy) -> anyhow::Result<()> {
        ...
        process::run(&mut StdCommand::new("launchctl").arg("unload").arg(&unit_path))?;
        if unit_path.exists() {
            found = true;
            log::info!(target: "edgedb::server::destroy",
...
impl LocalInstance<'_> {
    ...
    fn get_slot(&self) -> anyhow::Result<&String> {
        self.slot.get_or_try_init(|| match &self.get_meta()?.slot {
            Some(s) => Ok(s.clone()),
        ...
        if meta.version.is_nightly() {
            ...
            self.current_version
                .get_or_try_init(|| {
                    Ok(self
                        .method
                        .get_version(&meta.version.to_query())?
                        .version()
                        .clone())
                })
                .map(Some)
    ...
    fn upgrade(&self, meta: &Metadata) -> anyhow::Result<InstanceRef<'_>> {
        Ok(LocalInstance {
            ...
            slot: Lazy::eager(
                meta.slot
                    .as_ref()
                    .expect("macos packages always have a slot")
                    .clone(),
            ),
            current_version: Lazy::eager(
                meta.current_version
                    .as_ref()
                    .expect("current version is known during upgrade")
                    .clone(),
            ),
...
pub fn launchd_plist_path(name: &str, system: bool) -> anyhow::Result<PathBuf> {
    Ok(plist_dir(system)?.join(plist_name(name)))
}

fn plist_data(name: &str, meta: &Metadata) -> anyhow::Result<String> {
    let system = false;
    let path = get_server_path(
        meta.slot
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("no slot on MacOS"))?,
    );
    Ok(format!(
        r###"
...
pub fn launchctl_status(name: &str, _system: bool, cache: &StatusCache) -> Service {
    use Service::*;
    let list = cache
        .launchctl_list
        .get_or_init(|| process::get_text(&mut StdCommand::new("launchctl").arg("list")));
    let txt = match list {
        Ok(txt) => txt,
        Err(e) => {
    ...
    Inactive {
        error: format!("service {:?} not found", svc_name),
    }
}
...
pub fn create_launchctl_service(name: &str, meta: &Metadata) -> anyhow::Result<()> {
    ...
    process::run(StdCommand::new("launchctl").arg("load").arg(plist_path))?;
    Ok(())
}
