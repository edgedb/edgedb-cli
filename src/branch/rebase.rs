...
pub async fn main(options: &Rebase, context: &Context, connection: &mut Connection, cli_opts: &Options) -> anyhow::Result<()> {
    ...
    eprintln!("Removing old feature branch..");
    ...
    rename_temp_to_feature(&temp_branch, &context.auto_config.current_branch, cli_opts, &mut temp_branch_connection).await?;
