...
pub fn init(builder: &mut env_logger::Builder, opt: &Options) {
    ...
    match &opt.subcommand {
        ...
        Some(Command::Common(Common::CreateMigration(c))) if c.debug_print_queries => {
        ...
        Some(Command::Server(s)) => match &s.subcommand {
            Server::Uninstall(u) if u.verbose => {
                builder.filter_module("edgedb::server::uninstall", log::LevelFilter::Info);
            }
            Server::Upgrade(u) if u.verbose => {
                builder.filter_module("edgedb::server::upgrade", log::LevelFilter::Info);
            }
            Server::Destroy(d) if d.verbose => {
                builder.filter_module("edgedb::server::destroy", log::LevelFilter::Info);
            }
            _ => {}
        },
        _ => {}
