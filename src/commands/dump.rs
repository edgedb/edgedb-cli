...
impl Guard {
    async fn open(filename: &Path) -> anyhow::Result<(Output, Guard)> {
        if filename.to_str() == Some("-") {
            ...
            let tmp_path = filename.with_file_name(tmp_file_name(filename.as_ref()));
...
async fn dump_db(
    ...
    match msg {
        ServerMessage::DumpHeader(packet) => {
            ...
            header_buf.push(b'H');
            header_buf.extend(&sha1::Sha1::from(&packet.data).digest().bytes()[..]);
            header_buf.extend(&(packet.data.len() as u32).to_be_bytes()[..]);
            output.write_all(&header_buf).await?;
            output.write_all(&packet.data).await?;
        }
        ServerMessage::ErrorResponse(err) => {
            seq.err_sync().await.ok();
            return Err(anyhow::anyhow!(err).context("Error receiving dump header"));
        }
        _ => {
    ...
    loop {
        ...
        match msg {
            ...
            ServerMessage::ErrorResponse(err) => {
                seq.err_sync().await.ok();
                return Err(anyhow::anyhow!(err).context("Error receiving dump block"));
            }
            _ => {
                return Err(anyhow::anyhow!("WARNING: unsolicited message {:?}", msg));
            }
