...
async fn set_bool(cli: &mut Connection, name: &str, value: &ConfigBool)
    -> Result<(), anyhow::Error>
{
    assert!(matches!(&value.value[..], "true" | "false"));
    print::completion(&cli.execute(
    ...
    -> Result<(), anyhow::Error>
{
    ...
    match &cfg.command {
        ...
        C::Set(Set { parameter: S::AllowDmlInFunctions(param) }) => {
            set_bool(cli, "allow_dml_in_functions", param).await
        }
        C::Reset(Res { parameter }) => {
            ...
            let name = match parameter {
                ...
                C::AllowDmlInFunctions => "allow_dml_in_functions",
            };
            print::completion(&cli.execute(
