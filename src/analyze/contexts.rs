...
impl<'a> Gather<'a> {
    ...
    fn scan_shape(&mut self, shape: &'a Shape) {
        for ctx in &shape.contexts {
            ...
            self.by_location
                .entry((ctx.buffer_idx, ctx.start))
...
pub fn preprocess(input: AnalysisData) -> Analysis {
    ...
    let mut index = 0;
    let mut buffers_ctx = input.buffers.iter().map(|_| Vec::new()).collect::<Vec<_>>();
    for (&(buf, offset), &(len, ref items)) in &by_location {
        if let Some(&num) = items.iter().flat_map(|c| contexts.get(c)).next() {
            if let Some(ctxs) = buffers_ctx.get_mut(buf) {
                ctxs.push(ContextSpan { offset, len, num })
            }
            continue;
        }
        ...
        for c_id in items {
            if !contexts.contains_key(c_id) {
                // TODO(tailhook) use try_insert
