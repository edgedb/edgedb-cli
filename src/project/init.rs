...
use crate::platform::{home_dir, path_bytes, symlink_dir, tmp_file_path};
...
use crate::server::init::{self, allocate_port, try_bootstrap};
use crate::server::install::{self, exit_codes, optional_docker_check};
use crate::server::is_valid_name;
use crate::server::methods::{InstallMethod, InstallationMethods, Methods};
use crate::server::options::StartConf;
use crate::server::os_trait::{InstanceRef, Method};
...
fn ask_method(available: &InstallationMethods, options: &Init) -> anyhow::Result<InstallMethod> {
...
fn ask_name(methods: &Methods, dir: &Path, options: &Init) -> anyhow::Result<String> {
...
fn ask_version(meth: &dyn Method, options: &Init) -> anyhow::Result<DistributionRef> {
    ...
    let distribution = meth
        .get_version(&ver_query)
        .map_err(|e| {
...
fn init_existing(options: &Init, project_dir: &Path) -> anyhow::Result<()> {
    ...
    let distr = meth.get_version(&ver_query).map_err(|e| {
        eprintln!(
            "edgedb error: \
                Cannot find EdgeDB version {}: {}",
            ver_query, e
        );
        eprintln!(
            "  Hint: try different installation method \
...
fn hash(path: &Path) -> anyhow::Result<String> {
    Ok(hex::encode(
        sha1::Sha1::from(path_bytes(path)?).digest().bytes(),
    ))
}
...
fn write_stash_dir(dir: &Path, project_dir: &Path, instance_name: &str) -> anyhow::Result<()> {
...
fn init_new(options: &Init, project_dir: &Path) -> anyhow::Result<()> {
    eprintln!(
        "`edgedb.toml` is not found in `{}` or above",
        project_dir.display()
    );
