...
use crate::migrations::create::{execute, execute_start_migration};
use crate::migrations::create::{execute_if_connected, write_migration};
...
pub async fn main(cli: &mut Connection, _options: &Options,
...
async fn needs_fixup(cli: &mut Connection, ctx: &Context)
    -> anyhow::Result<bool>
{
...
async fn create_revision(cli: &mut Connection, ctx: &Context,
    create: &CreateMigration)
...
async fn want_fixup() -> anyhow::Result<bool> {
    ...
    echo!("");
    echo!("Feel free to skip this step if you don't have \
...
impl TwoStageRemove<'_> {
    ...
    async fn rename_fixups(&mut self, revs: impl IntoIterator<Item=&str>)
        -> anyhow::Result<()>
    {
        ...
        let mut queue: Vec<_> = revs.into_iter()
            .map(|r| r.to_owned())
            .collect();
    ...
    async fn rename(&mut self, path: &Path) -> anyhow::Result<()> {
        ...
        let mut tmp_fname = path.file_name()
