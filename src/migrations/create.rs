...
use edgeql_parser::tokenizer::{Kind as TokenKind, TokenStream};
...
async fn execute(cli: &mut Connection, text: impl AsRef<str>) -> anyhow::Result<()> {
...
async fn query_row<R>(cli: &mut Connection, text: &str) -> anyhow::Result<R>
where
    R: Queryable,
{
    let text = text.as_ref();
    ...
    let descr = loop {
        ...
        if let Some(proposal) = data.proposed {
            if proposal.confidence >= SAFE_CONFIDENCE || options.allow_unsafe {
                ...
                eprintln!(
                    "But confidence is {} (minimum is {})",
                    proposal.confidence, SAFE_CONFIDENCE
                );
                anyhow::bail!(
                    "Server cannot make decision. Please run in \
...
async fn run_interactive(
    ...
    let descr = 'migration: loop {
        let descr =
            query_row::<CurrentMigration>(cli, "DESCRIBE CURRENT MIGRATION AS JSON").await?;
        if descr.complete {
            break descr;
        }
        if let Some(proposal) = &descr.proposed {
                ...
                loop {
                    match choice(prompt).await? {
                        ...
                        No => {
                            ...
                            save_point += 1;
                            execute(cli, format!("DECLARE SAVEPOINT migration_{}", save_point))
                                .await?;
                            continue 'migration;
                        }
                        List => {
                        ...
                        Confirmed => {
                            if descr.confirmed.is_empty() {
                                println!("No EdgeQL statements were confirmed yet");
                            } else {
                                println!("Following EdgeQL statements were confirmed:");
                ...
                match execute(cli, &text).await {
                    ...
                    Err(e) => {
                        match e.downcast::<ErrorResponse>() {
                            ...
                            Err(err) => {
                                eprintln!("Error applying statement: {:#}", err);
                            }
                        }
            ...
            if let Some(prompt_id) = &proposal.prompt_id {
                operations.push(operations.last().unwrap().insert(prompt_id.clone()).0);
            } else {
...
pub async fn create(
    ...
    let descr = query_row::<CurrentMigration>(cli, "DESCRIBE CURRENT MIGRATION AS JSON").await?;
    ...
    if db_migration != migrations.keys().last() {
        anyhow::bail!(
            "Database must be updated to the last migration \
    ...
    let exec = if create.non_interactive {
        run_non_interactive(&ctx, cli, migrations.len() as u64 + 1, &create).await
    } else {
        if create.allow_unsafe {
            log::warn!("The `--allow-unsafe` flag is unused in interactive mode");
        }
...
fn add_newline_after_comment(value: &mut String) -> Result<(), anyhow::Error> {
    let last_token = TokenStream::new(value)
        .last()
...
fn get_user_input(req: &[RequiredUserInput]) -> Result<BTreeMap<String, String>, anyhow::Error> {
